...
import { getFocusSessionById } from "@/server/repositories/focus-session-repository";
import {
...
export default async function LearningLogsPage({ searchParams }: LearningLogsPageProps) {
  ...
  if (sessionId) {
    ...
    const existingLog = await getLearningLogForFocusSession(userId, sessionId);
    if (existingLog) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] max-w-md mx-auto text-center space-y-4">
  ...
  const pendingSessions = await getCompletedSessionsWithoutLogs(userId);
