...
import { getRevisionMetrics, getRevisionsForUser } from "@/server/repositories/revision-repository";
import { getMoneySummary } from "@/server/repositories/money-repository";
...
export default async function DashboardPage() {
  ...
  const [summary, revisionMetrics, dueRevisions, moneySummary] = await Promise.all([
    getDailyTaskSummary(userId, todayStr),
    getRevisionMetrics(userId, todayStr),
    getRevisionsForUser(userId, "due", todayStr),
    getMoneySummary(userId, now.getFullYear(), now.getMonth() + 1),
  ]);
